...
void rc4Init(unsigned char* key, int length){
	...
	for(rci = 0; rci < 256; rci++){
          rcj = (rcj + state[rci] + key[rci % length]) % 256;
	  swap(state[rci], state[rcj]);
	} 
...
unsigned char rc4Next(void){
	rci = (rci + 1) % 256;
	rcj = (rcj + state[rci]) % 256;
	swap(state[rci], state[rcj]);
	
	return state[(state[rci] + state[rcj]) % 256];
}
...
*/      
	...
	//copy message data from kernal space into user space (buf)
	//count is num of bytes of message
	if(copy_to_user(buf, keystream, count)){
	  //copy_to_user returns 0 on success, if it fails it will return the exception
	   kfree(keystream);
	   return -EFAULT; //bad address exception dealio
	}
	//printk("Read buffer content: ");
	printk("Value of count: %d" + count); 
	for(i = 0; i < count; i++){
	  //printk("%d ", buf[i]);
	}
        printk("\n");
...
ssize_t myRand_write(struct file*filp, const char __user *buf, size_t count, loff_t *fpos){
  /*
	...
	BE SURE NOT TO DIRECTLY DEREFERENCE A USER POINTER!
   */
	printk("Value of count in write: %d" + count); 
	unsigned char* key; 
	...
	//message is kernal space, buf is user space
	if(copy_from_user(key, buf, count)){
	   kfree(key);
           return -EFAULT;
	}

	//reinitialize RC4 generator with user input
	printk("Write - reinitialize rc4\n");
        rc4Init(key, count);
...
myRand_init_module(void){
   ...
   */
   ...
   unsigned char* key;
   key = (unsigned char*) kmalloc(1, GFP_KERNEL);
   rc4Init(key, 1);
